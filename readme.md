...
│   ├── assets/                   # Marketing images + site JS/CSS (aish.png, createBookmarklet.svg, main.js, styles.css)
...
The generator's source lives in `site-src/pages/index.html` (markup) and `docs/assets/main.js` (the `/* ── Bookmarklet generator */` component). The legacy standalone `bookmarklet-generator/` folder is no longer deployed (GitHub Pages serves from `docs/`).
